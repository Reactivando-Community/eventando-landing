  ...
  const handleSubmit = async () => {
    ...
    try {
      await hubCommunity.swForm.create({
        ...
        email: email.trim(),
        college: "Prestação de Contas",
        college_course: "Certificado",
      });
      ...
      if (
        ...
        setError("Esse CPF já possuí um registro.");
    ...
    return (
      <div className="bg-yellow-400 border-4 border-black shadow-[12px_12px_0_#fff] p-8 md:p-12 text-center transform md:rotate-1">
        ...
        <p className="text-black font-bold text-lg md:text-xl max-w-md mx-auto">
          Recebemos suas informações com sucesso. Em breve seu certificado estará disponível.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white border-4 border-black p-8 md:p-12 shadow-[12px_12px_0_#9333ea]">
      ...
      <p className="text-gray-800 font-bold mb-8 text-sm md:text-base border-l-4 border-black pl-4">
        Preencha com exatidão para emissão do seu certificado e para compor nossa prestação de contas.
      </p>
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-start gap-3 mt-4">
          <input
            ...
            className="mt-1 w-5 h-5 border-2 border-black bg-[#f4f4f0] text-black focus:ring-0 focus:ring-offset-0 cursor-pointer accent-black"
          />
          <label htmlFor="terms" className="text-sm font-bold text-gray-800 cursor-pointer">
            Concordo com os{" "}
            <a
              href="/startup-weekend/termos"
              target="_blank"
              rel="noopener noreferrer"
              className="text-black underline decoration-2 underline-offset-2 hover:text-blue-600 transition-colors"
            >
